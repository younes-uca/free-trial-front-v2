...
private setValidation(value: boolean){
    ...
    public saveWithShowOption(showList: boolean){
     this.freeTrialService.save().subscribe(freeTrial=>{
        if(freeTrial != null){
           this.freeTrials.push({...freeTrial});
           this.createFreeTrialDialog = false;
           this.submitted = false;
           this.selectedFreeTrial = new FreeTrialDto();

        }else{
            this.messageService.add({severity: 'error', summary: 'Erreurs',detail: 'Free trial existe déjà' });
        }
    ...
    private validateForm(): void{
       ...
       public async openCreateLevel(level: string) {
          const isPermistted = await this.roleService.isPermitted('Level', 'add');
         if(isPermistted) {
         this.selectedLevel = new LevelDto();
       ...
       public async openCreateTeacher(teacher: string) {
          const isPermistted = await this.roleService.isPermitted('Teacher', 'add');
         if(isPermistted) {
         this.selectedTeacher = new TeacherDto();
